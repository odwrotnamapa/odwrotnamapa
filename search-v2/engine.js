(function () {
  ...
  function maxMergeDistanceMeters(similarity) {
    if (similarity >= 0.97) return 150;
    if (similarity >= 0.92) return 90;
    if (similarity >= 0.88) return 45;
    return 0;
  }
  ...
  function merge(results) {
    ...
    for (const result of results) {
      ...
      const existingIndex = unique.findIndex(existing => {
        ...
        const similarity = Ranker.similarity(
          name,
          existing.name ||
        ...
        const distanceMeters = metersBetween(
          Number(existing.lat), Number(existing.lon),
          lat, lon
        );

        return distanceMeters <= maxMergeDistanceMeters(similarity);
      });
